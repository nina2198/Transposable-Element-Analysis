...
# Read as plain tab-delimited tables
...
# Assign standard GFF column names (first 9 columns only)
...
droso_edta <- droso_edta %>%
  # Avoid factor pitfalls in joins/mapping later
  mutate(
...
```{r}
# ============================================================
# FINAL FULL HARMONISATION CODE (EDTA + Earl Grey) — Wicker et al. (2007)
#
# Implements your rules:
#   - one canonical `order` column
#   - decode EDTA 3-letter codes (DTM, DTT, etc.) + suffix codes (DNA/DTM etc.)
#   - treat MITE and DNAnona as flags (not taxonomic ranks)
#   - treat I-Jockey as a single LINE clade (no split to family)
#   - propagate superfamily -> order -> class when unambiguous
#   - use literal "Unknown" everywhere (case/whitespace normalised)
#   - Earl Grey special-case: DNA/CMC-Transib -> order=TIR, superfamily=Transib, family=CMC, class II
#   - conflict resolution: retro-order + DNA/TIR superfamily -> force TIR/Class II
#   - invariant enforcement + sanity checks
#
# EXPECTS INPUT OBJECTS:
#   droso_earlG_TEonly with columns: type, attributes (optional)
#   droso_edta_TEonly  with column:  classification
# ============================================================
...
# ------------------------------------------------------------
# HARD normalisation: every unknown-like value becomes exactly "Unknown"
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# EDTA: map standalone 3-letter code -> multi-level type
# preserve trailing text as family using "-"
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# EDTA: decode suffix codes in multi-level strings (DNA/DTM, LTR/RLG, LINE/RII)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Earl Grey attributes: extract NAME= (family fallback only)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Split type string into prefix/suffix
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# suffix -> superfamily/family
#   - Keep I-Jockey intact
#   - Earl Grey special-case: CMC-Transib -> sf=Transib, fam=CMC
#   - EDTA: allow "-" split into sf/family (Gypsy-FamX)
#   - Earl Grey: do NOT split hyphens (hAT-hATm etc.)
# ------------------------------------------------------------
...
infer_sf_fam <- function(suffix, allow_family_split = TRUE) {
  ...
  # ------------------------------------------------------------
  # NEW: targeted splits where "-" indicates FAMILY, not part of the superfamily label
  # (Does NOT affect Bel-Pao because it doesn't match these prefixes.)
  # Examples:
  #   TcMar-Pogo -> sf_raw=TcMar, fam_raw=Pogo   -> canonicalise => sf=Tc1-Mariner, fam=Pogo
  #   TcMar-Tc1  -> sf_raw=TcMar, fam_raw=Tc1   -> canonicalise => sf=Tc1-Mariner, fam=Tc1
  #   hAT-Ac     -> sf_raw=hAT,  fam_raw=Ac
  #   R1-R1A     -> sf_raw=R1,   fam_raw=R1A
  # ------------------------------------------------------------
...
# ------------------------------------------------------------
# Canonicalise sf/fam (minimal safe normalisations)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Infer order from prefix + superfamily (Wicker-safe)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Promote upward: superfamily -> order (only unambiguous); order -> class (always)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Resolve clear conflicts: retro-order + DNA/TIR superfamily -> force TIR/Class II
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# EDTA: final recode pass to remove residual code-like superfamilies
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Invariant: if superfamily is known, order + class must be known
# ------------------------------------------------------------
...
standardise_multi_level_type <- function(df, type_col, type_source_label, attr_col = NULL) {
  ...
  if (type_source_label == "EDTA:classification") {
    ...
    df2 <- df2 %>%
      mutate(
        ...
        # Preserve Helitron if explicitly indicated; otherwise keep ranks Unknown
        order = case_when(
        ...
        # MITE: force into TIR, keep informative sf
  ...
  # EDTA: final recode pass for residual codes
  if (type_source_label == "EDTA:classification") {
...
# A) Every unknown-like token must be exactly "Unknown" (case + whitespace)
stopifnot(
  !any(str_to_lower(str_trim(droso_edta_std$superfamily)) == "unknown" & str_trim(droso_edta_std$superfamily) != "Unknown"),
  !any(str_to_lower(str_trim(droso_earlG_std$superfamily)) == "unknown" & str_trim(droso_earlG_std$superfamily) != "Unknown")
)

# B) Earl Grey: CMC-Transib mapping exactly as requested
cmc <- droso_earlG_std %>% filter(str_detect(type_raw, regex("^DNA/CMC-Transib$", TRUE)))
stopifnot(nrow(cmc) > 0)
stopifnot(all(cmc$order == "TIR" & cmc$superfamily == "Transib" & cmc$family == "CMC" & cmc$class_I_II == "II"))

# C) EDTA: no residual codes as superfamily
stopifnot(!any(droso_edta_std$superfamily %in% c("DTA","DTC","DTH","DTM","DTT")))

# D) Invariant proof (should be 0)
stopifnot(
  droso_earlG_std %>%
    filter(superfamily != "Unknown" & (order == "Unknown" | class_I_II == "Unknown" | class_broad == "Unknown")) %>%
    nrow() == 0,
  droso_edta_std %>%
    filter(superfamily != "Unknown" & (order == "Unknown" | class_I_II == "Unknown" | class_broad == "Unknown")) %>%
    nrow() == 0
