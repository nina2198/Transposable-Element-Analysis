...
```{r include=FALSE}
setwd("~/Desktop/Thesis/final_analysis")
```
...
# separation of the type column and reclassification where needed

# ============================================================
# Earl Grey (EG) only: standardise multi-level `type` into
# class/order/superfamily/family with a single `order` column.
# - Keeps original `type`
# - Adds: eg_prefix, eg_suffix, class_broad, class_I_II, order,
#         superfamily, family, mechanism, reclassified
# - Uses attributes as FAMILY-only fallback (NAME=... by default)
# - Special-case: "I-Jockey" is kept as a single LINE clade label
...
# IMPORTANT: do NOT split I-Jockey into family (both are LINE clades)
...
# Infer order from prefix + superfamily (single order column)
...
standardise_eg <- function(df, family_attr_key = "NAME") {
  ...
  # FAMILY fallback from attributes (limited to NAME=... by default)
  ...
  # Repeat Unknown for Unknown type rows across taxonomy fields
  df %>%
    mutate(
